...
contract ServiceRemark{
    ...
    // 申请服务
    function applyService(string _id, string _orgId) public returns(int) 
    {
        int _state = 0; // 0表示已申请
    ...
    function confirmService(string _id, int _state) public returns(int) 
    {
        ...
        Entry entry = table.newEntry();
        entry.set("state", _state); // 1表示已接受，2表示已完成
    ...
    // 组织拒绝服务
    function rejectService(string _id, string _endTime, string _comment) public returns(int) 
    {
        int _state = 3; // 3表示已拒绝
